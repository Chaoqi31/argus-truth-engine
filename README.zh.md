...
AI 治理团队和买方分析师。你交给它一份 PDF 或粘贴一段文本；它抽取每一个事实性声明，用
...
一位分析师拿到第三方 RAG 系统生成的研究报告。这些都不是他们写的；但他们都得在行动前先
*信任*它。
...
它审计的是那些你并未亲自生成、但仍然需要信任的 AI 内容。

## Argus 做什么

你给它任何一份 AI 生成的文档 —— PDF、备忘录、研究报告或聊天记录。它还给你**每一个事实性
声明**、**每条声明的判决**，以及背后的**推理和证据**。
...
每条发现都同时包含一段整理后的**推理链**（行动 / 观察 / 推理）、claim 级证据面板和完整的
**10-stage trace** —— pipeline 实际记录下来的每一次搜索、抓取和验证步骤。
...
Trace 使用**渐进披露**：每个 claim 先以一行展示（verdict + 步数）；展开后阅读推理，再打开
任意搜索或来源链接查看底层证据。**实时审查**通过 WebSocket 流式渲染每一步；**示例审查**
回放同一段真实 trace。
...
成本更低、不联网）或是**确定性**（parse、审核闸门、置信度），把 MiroMind 的预算只花在真正
需要开放网络的两步上。
...
# 打开 http://127.0.0.1:3000 → 点击 "See a sample audit" 回放一次真实录制的审计，无需 API key。
```
